...
//Scale and translate image onto camera space
  Vector3D originalPos = Vector3D(x, y, 0);
    
//Scale pixel
  originalPos.x = originalPos.x * 2 * tan(0.5 * (PI/180) *hFov);
  originalPos.y = originalPos.y * 2 * tan(0.5 * (PI/180) *vFov);
    
//Translate pixel (onto camera space)
  originalPos.x = originalPos.x - tan(0.5 * (PI/180) * hFov);
  originalPos.y = originalPos.y - tan(0.5 * (PI/180) * vFov);
  originalPos.z = -1;

//Converts camera view to world view
  Vector3D convertedPos = c2w * originalPos;
  convertedPos.normalize();
  Ray returnRay = Ray( pos, convertedPos,fClip, -1);
  returnRay.min_t = nClip;
  return returnRay;
